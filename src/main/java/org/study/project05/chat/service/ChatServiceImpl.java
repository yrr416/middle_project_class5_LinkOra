...
public class ChatServiceImpl implements ChatService {
    ...
    private final org.study.project05.space.mapper.FacilityMapper facilityMapper;
    ...
    public ChatVO processMessage(ChatVO chatVO) {
        ...
        systemMsg.put("content",
                ...
                "[강력 준수 지침 - 예약 및 취소]\n" +
                "1. **실시간 정보 동기화 (Smart Prefill)**: 대화 도중 날짜, 시간, 인원수가 언급되면 즉시 `[[PREFILL:yyyy-MM-dd|시작|종료]]` 태그를 답변 끝에 포함해.\n" +
                "2. **명령 실행 필수 (중요)**: 예약을 최종 확정할 때는 `[[COMMIT_BOOKING:공간ID|시작T시각|종료T시각|인원]]`를, 취소할 때는 `[[CANCEL_BOOKING:예약ID]]` 태그를 답변에 **반드시** 포함해야 시스템에 반영돼. 태그 없이 말로만 성공했다고 하지 마.\n" +
                "3. **확정 유도**: 텍스트 확정보다는 카드 UI의 **'바로예약' -> '공간 예약하기'** 순서로 유도해.\n" +
                "\n[새로운 추천 시나리오 - 공간 추천]\n" +
                "1. **위치 기반 추천**: 사용자가 '공간 추천'을 요청하면 [공간 정보 컨텍스트]에서 가장 상단에 있는(가까운) 3개의 공간을 `[[ACTIONS:공간ID|공간명|지점명|이미지|시설요약|가격]]` 태그로 보여줘.\n" +
    ...
    private String getReservationContext(Double lat, Double lng) {
        try {
            ...
            List<org.study.project05.space.vo.BranchVO> branches;
            ...
            for (org.study.project05.space.vo.BranchVO b : branches) {
                ...
                for (org.study.project05.space.vo.SpaceVO s : spaces) {
                    // 시설 정보 가져오기
                    org.study.project05.space.vo.FacilityVO f = facilityMapper.selectBySpaceIdx(s.getSpcIdx());
                    String facInfos = (f != null) ? getFacilitySummary(f) : "기본 시설";

                    // 가용성 체크 (단순화: 오늘 14시 기준 좌석 유무)
                    int maxCap = Integer.parseInt(s.getSpcMaxCapacity());
                    ...
                    sb.append("  * ").append(s.getSpcName())
                            ...
                            .append(", 가격:").append(s.getSpcPrice() != null ? s.getSpcPrice() : "0")
                            .append(")\n");
                }
            }
            return sb.toString();
    ...
    private String getFacilitySummary(org.study.project05.space.vo.FacilityVO f) {
