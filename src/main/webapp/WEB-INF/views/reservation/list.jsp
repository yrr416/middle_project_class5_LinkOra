                ...
                <div class="col-md-2">
                    ...
                    <select name="space_filter" class="form-select form-select-sm">
                        ...
                        <c:forEach var="sp" items="${spaceList}">
                            <option value="${sp.s_idx}"
                                    ${searchVO.space_filter == sp.s_idx.toString() ? 'selected':''}>
                                ${sp.s_name}
                            </option>
                        </c:forEach>
                    </select>
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <c:forEach var="r" items="${reservationList}">
                            <!-- 행 클릭 시 상세 모달 오픈 -->
                            <tr onclick="openDetailModal(${r.r_idx})" style="cursor:pointer;">
                                <td class="ps-4 text-muted">#${r.r_idx}</td>
                                <td>
                                    <div class="fw-semibold">${r.u_name}</div>
                                    <div class="text-muted small">${r.u_phone}</div>
                                </td>
                                <td>
                                    <div class="fw-semibold">${r.s_name}</div>
                                    <div class="text-muted small">${r.s_type}</div>
                                </td>
                                <td class="text-muted small">${r.b_name}</td>
                                <td>
                                    <div class="small">${r.r_start_time}</div>
                                    <div class="small text-muted">~ ${r.r_end_time}</div>
                                </td>
                                <td class="text-center">${r.r_headcount}명</td>
                                <td>
                                    <strong>
                                        ₩ <fmt:formatNumber value="${r.r_total_price}" type="number"/>
                                    </strong>
                                </td>
                                <td>
                                    <%-- 예약 상태 배지 --%>
                                    <c:choose>
                                        <c:when test="${r.r_status == 'PENDING'}">
                                ...
                                <td class="text-center" onclick="event.stopPropagation()">
                                    <button class="btn btn-outline-primary btn-sm py-0 px-2"
                                            onclick="openDetailModal(${r.r_idx})">
                                        <i class="bi bi-eye"></i>
                                    </button>
                                </td>
                            </tr>
                ...
                <div id="modalContent" style="display:none;">
                    ...
                    <div id="cancelForm">
                        ...
                        <form method="post" action="/admin/reservation/cancel" id="cancelFormTag">
                            ...
                            <input type="hidden" name="r_idx"       id="cancel_r_idx">
...
function openDetailModal(rIdx) {
    ...
    /* AJAX 상세 조회 */
    fetch('/admin/reservation/detail?r_idx=' + rIdx)
        .then(res => {
...
function renderModal(d) {
    ...
    /* ① 예약 기본 정보 채우기 */
    document.getElementById('d_r_idx').textContent       = '#' + d.r_idx;
    document.getElementById('d_r_start_time').textContent = d.r_start_time || '-';
    document.getElementById('d_r_end_time').textContent   = d.r_end_time   || '-';
    document.getElementById('d_r_headcount').textContent  = (d.r_headcount || 0) + '명';
    document.getElementById('d_r_total_price').textContent = '₩ ' + fmtNum(d.r_total_price);

    /* 취소 상태일 때는 r_content 를 "취소 사유"로 표시 */
    const contentLabel = d.r_status === 'CANCELLED' ? '[취소 사유] ' : '';
    document.getElementById('d_r_content').textContent = contentLabel + (d.r_content || '-');

    /* 상태 배지 */
    const statusInfo = STATUS_MAP[d.r_status] || { label: d.r_status, css: '' };
    ...
    /* ② 예약자 정보 */
    document.getElementById('d_u_name').textContent  = d.u_name  || '-';
    document.getElementById('d_u_email').textContent = d.u_email || '-';
    document.getElementById('d_u_phone').textContent = d.u_phone || '-';

    /* ③ 공간 정보 */
    document.getElementById('d_s_name').textContent  = d.s_name  || '-';
    document.getElementById('d_s_type').textContent  = d.s_type  || '-';
    document.getElementById('d_b_name').textContent  = d.b_name  || '-';
    document.getElementById('d_s_price').textContent = '₩ ' + fmtNum(d.s_price) + ' / 건';

    /* ④ 이력 타임라인 렌더링 */
    renderTimeline(d);

    /* ⑤ 취소 폼 — r_idx 및 환불 금액 미리 세팅 */
    document.getElementById('cancel_r_idx').value      = d.r_idx;
    document.getElementById('cancelRefundAmount').textContent = '₩ ' + fmtNum(d.r_total_price);
...
function renderTimeline(d) {
    ...
    const statusInfo = STATUS_MAP[d.r_status] || { label: d.r_status };

    /* 이벤트 목록 구성 */
    const events = [];

    /* 예약 접수 (항상 첫 번째) */
    events.push({ time: d.r_created, text: '예약 접수 (PENDING)', dot: '' });

    /* 마지막 상태 변경 (r_updated 가 r_created 와 다를 때만 표시) */
    if (d.r_updated && d.r_updated !== d.r_created) {
        let dotClass = '';
        let eventText = '상태 변경 → ' + statusInfo.label;
        if (d.r_status === 'CANCELLED') { dotClass = 'dot-cancel';   eventText = '강제 취소 처리'; }
        if (d.r_status === 'CONFIRMED') { dotClass = 'dot-confirm';  eventText = '예약 확정'; }
        if (d.r_status === 'COMPLETED') { dotClass = 'dot-complete'; eventText = '이용 완료 처리'; }
        events.push({ time: d.r_updated, text: eventText, dot: dotClass });
    }

    /* 현재 상태 마커 (PENDING 이면 "처리 대기 중") */
    if (d.r_status === 'PENDING') {
...
function renderFooterButtons(d) {
    ...
    if (d.r_status === 'PENDING') {
        html = '<form method="post" action="/admin/reservation/confirm" class="d-inline">' +
               '  <input type="hidden" name="r_idx"         value="' + d.r_idx + '">' +
    ...
    if (d.r_status === 'CONFIRMED' || d.r_status === 'USING') {
    ...
    if (['PENDING', 'CONFIRMED', 'USING'].includes(d.r_status)) {
